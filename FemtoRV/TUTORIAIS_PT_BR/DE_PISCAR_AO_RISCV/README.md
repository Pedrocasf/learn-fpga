# De Piscante ao RISC-V

Este tutorial é uma jornad progressiva de um simples design de piscador até um nucleo RISC-V.

Ele funciona nas seguintes placas:
- IceSugar
- IceStick
- IceBreaker
- ULX3S
...
Se você não tem uma placa , você pode rodar tudo em uma simulação (mas não é tão divertido).

## Introdução e referências em design de processadores

Para entender design de processadores a primeira coisa a ser lida é
[esta resposta](https://stackoverflow.com/questions/51592244/implementation-of-simple-microprocessor-using-verilog/51621153#51621153)
no Stackoverflow, que achei inspiradora. Também recomendo [este artigo](http://www.fpgacpu.org/papers/xsoc-series-drafts.pdf) sugerido por @mithro.
Para um curso completo recomendo [este do MIT](http://web.mit.edu/6.111/www/f2016/), este também 
dá os principios para ir muito alem do que foi feito aqui (pipelines etc...).

Para o básico de Verilog e sntaxe recomendase ler _Verilog by example by Blaine C. Readler_ este tambem é curto e consiso.

Existem duas coisas boas sobre a resposta do Stackoveflow:
- Ele vai ao essencial e mantem apenas o essencial.
- o exemplo dado é uma cpu RISC que compartilha diversas similaridades com RISC-V
  (exceto que este tem sinalizadores de estado, que RISC-V não tem).

O que nós aprendemos é que existe um _banco de registradores_ que guarda 
os chamodos _registradores de proposito geral_. Por proposito geral entend-se 
que cada vez que uma instrução lê ou escreve e mum registrador pode ser qualquer um deles
ao contrário de x86 (CISC) que tem 
